...
export function StartGameForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        const msg =
          typeof data.error === "string"
            ? data.error
            : "Could not start the game. Check your name or email and PIN.";
        setError(msg);
        setPending(false);
        return;
      }
      ...
      if (sign?.error) {
        setError("Session created but sign-in failed. Try logging in from the home page.");
        setPending(false);
        return;
      }
  ...
  return (
    <form onSubmit={onSubmit} className="flex max-w-md flex-col gap-4">
      ...
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        Same PIN you set at registration or in Settings. You will be signed in on this device.
      </p>
      <button
        type="submit"
